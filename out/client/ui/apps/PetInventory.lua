...
local PET_ACTION_BUTTONS = TS.import(script, game:GetService("ReplicatedStorage"), "TS", "constants", "Pets").PET_ACTION_BUTTONS
...
local PetInventoryApp
do
	PetInventoryApp = Roact.Component:extend("PetInventoryApp")
	function PetInventoryApp:init(props)
	end
	function PetInventoryApp:render()
		...
		local arrayPetInventory = Object.values(clientStore:getState().petInventory)
		local _arg0_1 = function(pet)
			return Roact.createElement(PetButton, {
				pet = pet.type,
				uuid = pet.uuid,
				onClick = function()
					return self:clickPet(pet)
				end,
			})
		end
		-- ▼ ReadonlyArray.map ▼
		local _newValue_1 = table.create(#arrayPetInventory)
		for _k, _v in arrayPetInventory do
			_newValue_1[_k] = _arg0_1(_v, _k - 1, arrayPetInventory)
		end
		-- ▲ ReadonlyArray.map ▲
		...
		local _child = self.state.selectedPet and (Roact.createElement(PetInfoFrame, {
			pet = self.state.selectedPet.type,
			uuid = self.state.selectedPet.uuid,
		}))
		if _child then
	...
	function PetInventoryApp:clickPet(pet)
		if self.state.isDeleteMode then
		else
			self:setState(function(prev, props)
				return {
					selectedPet = pet,
				}
			end)
...
return {
	PetInventoryApp = PetInventoryApp,
}
